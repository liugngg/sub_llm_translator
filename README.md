...
- 生成单文件格式（Nuitka --使用upx 压缩）
  `uv run --with nuitka python -m nuitka --mingw64 --onefile --onefile-no-compression --plugin-enable=upx --lto=yes --show-progress --output-dir=dist --remove-output sub_trans.py`
